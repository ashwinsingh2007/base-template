...
export default function App() {
  ...
  const [filters, setFilters] = useState({ genre: "all", yearFrom: "", yearTo: "", rating: "", availability: "all" });
  ...
  const handleSubmit = (e) => {
    ...
    if (existingBook) {
      // Increment count if book with same title and author exists
      setBooks(
        books.map((book) =>
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        ...
        <Input type="number" name="yearFrom" placeholder="Year from" onChange={(e) => handleFilterChange("yearFrom", e.target.value)} className="w-full sm:w-auto" />
        <Input type="number" name="yearTo" placeholder="Year to" onChange={(e) => handleFilterChange("yearTo", e.target.value)} className="w-full sm:w-auto" />
        <Input type="number" name="rating" placeholder="Min rating" onChange={(e) => handleFilterChange("rating", e.target.value)} className="w-full sm:w-auto" step="0.1" min="0" max="5" />
      </div>
